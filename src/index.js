...
/**
 * @type {Map<number, { fileId: string, originalFileName: string, userName: string, mimeType?: string, step: 'await_category' | 'await_subcategory' | 'await_comment', category_id: string | null, category: string | null, subcategory_id: string | null, subcategory: string | null, sessionCreatedAt: number }>}
 */
 ...
 * @param {'accept' | 'question'} action
 */
async function applyAccountantExpenseAction(query, expenseId, action) {
  const status = action === 'accept' ? 'in_progress' : 'need_info';
  let row;
  try {
  ...
  const userText =
    action === 'accept'
      ...
      : '❓ По счёту есть вопрос. Напишите уточнение.';
  try {
    await bot.sendMessage(userChatId, userText);
